...
```bash
RagPipeline$ sam build --use-container
```
...
```bash
RagPipeline$ sam logs -n StockCheckerFunction --stack-name "ragpipeline" --tail
```
...
```bash
RagPipeline$ pip install -r tests/requirements.txt --user
# unit test
RagPipeline$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
RagPipeline$ AWS_SAM_STACK_NAME="ragpipeline" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "ragpipeline"
```
